...
warehouseRouter.post("/inventory/movements", requireAuth, (req, res) => {
  ...
  if (!result.ok) {
    const status = result.reason === "auth_required" ? 401 : result.reason === "item_not_found" ? 404 : result.reason === "forbidden" ? 403 : result.reason === "insufficient_stock" ? 409 : 400;
...
warehouseRouter.patch("/permissions/users/:userId", requireWarehouseAction("managePermissions"), (req, res) => {
  ...
  if (!result.ok) {
    ...
    res.status(status).json({ ok: false, message: "No fue posible actualizar los permisos directos del usuario." });
    return;
  }
...
warehouseRouter.patch("/users/:userId", requireWarehouseAction("manageUsers"), (req, res) => {
  ...
  if (!result.ok) {
    ...
    res.status(status).json({ ok: false, message: result.reason === "duplicate_email" ? "Ya existe un usuario con ese correo." : "No fue posible actualizar el usuario." });
    return;
  }
...
warehouseRouter.delete("/users/:userId", requireWarehouseAction("deleteUsers"), (req, res) => {
  ...
  if (!result.ok) {
    ...
    res.status(status).json({ ok: false, message: "No fue posible eliminar el usuario." });
    return;
  }
...
warehouseRouter.patch("/users/:userId/active", requireWarehouseAction("manageUsers"), (req, res) => {
  ...
  if (!result.ok) {
    const status = result.reason === "auth_required" ? 401 : result.reason === "user_not_found" ? 404 : result.reason === "forbidden" ? 403 : 400;
    res.status(status).json({ ok: false, message: "No fue posible cambiar el estado del usuario." });
    return;
  }
...
warehouseRouter.patch("/users/me/profile", (req, res) => {
  ...
  if (!result.ok) {
    ...
    res.status(status).json({ ok: false, message: result.reason === "duplicate_email" ? "Ya existe un usuario con ese correo." : result.reason === "self_edit_limit_reached" ? "La autoedicion ya fue utilizada." : "No fue posible actualizar el perfil." });
    return;
  }
